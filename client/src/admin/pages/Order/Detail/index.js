                    ...
                    orderDetail && orderDetail.List && orderDetail.List.map((item, index)=> {
                        return (
                            <Row key={index} className="order-item-value">
                                <Col>{index}</Col>
                                <Col>{item.id}</Col>
                                <Col><img src={item.SanPham_HinhAnh && `${process.env.REACT_APP_API_HOST_URL}/public/images/${item.SanPham_HinhAnh[0]}`} alt="hình ảnh"/></Col>
                                ...
                                <Col>{0}</Col>
                                <Col>{item.ThanhTien && item.ThanhTien.toLocaleString('en-US')} VND</Col>
                            </Row>
                        )
                    })
                }
