            ...
            setNotify((notify) => {
                if (response && (!response.success || response.data.length === 0)) {
                    return {
                        ...notify,
                        show: true,
                        message: "Không tìm thấy dữ liệu phân quyền",
                        success: false,
                    }
                }
                return { ...notify }
                            ...
                            <Form.Select name="city" onChange={changeAddressHandler}>
                                ...
                                {listCity &&
                                    listCity.map((city, index) => {
                                        return (
                                            <option key={index} value={`${city.code}-${city.name}`}>
                                                {city.name}
                                            </option>
                                        )
                                    })}
                    ...
                    <Row className="mb-3" style={{ width: "100%" }}>
                        ...
                        <Form.Group as={Col} md="4" controlId="user-permission">
                            ...
                            <Form.Select
                                aria-label="Default select example"
                                name="permission"
                                value={user.CapDoTaiKhoan}
                                onChange={onChangeInput}
                            >
                                <option value={user.CapDoTaiKhoan}>
                                    {user.CapDoTaiKhoan_Ten ? user.CapDoTaiKhoan_Ten : "User"}
                                </option>
                                {permission &&
