...
const CartComponent = () => {
    ...
    const ChangInput = async (e, item) => {
        ...
        const dataUpdate = { SoLuong: e.target.value, IDGioHang: item.IDGioHang, IDSanPham: item.IDSanPham }
    ...
    return (
                ...
                <>
                                            ...
                                            <tbody>
                                                {
                                                    cart && cart.map && cart.map((item, k) => (
                                                        <tr key={k}>
                                                            ...
                                                            <td className="shoping__cart__price">{Math.abs(item.SanPhamGiaGoc) ? Math.abs(item.SanPhamGiaGoc) * (Math.abs(item.SoLuong) ? Math.abs(item.SoLuong) : 1) : 0} VND</td>
                                                            <td className="shoping__cart__quantity">
                                                                <div className="quantity">
                                                                    <div className="pro-qty d-flex align-items-center justify-content-center m-auto fs-4">
                                                                        <input type="number" className="text-center" name="SoLuong" min={0} value={Math.abs(item.SoLuong) ? Math.abs(item.SoLuong) : 1} onChange={(e) => ChangInput(e, item)} />
                                                                    </div>
                                                                </div>
                                                            </td>
                                                            <td className="shoping__cart__item__close">
                                                                <span className="icon_close" onClick={() => handleDelete(item.IDGioHang, item.IDSanPham)} />
                                                            </td>
                                                        </tr>
                                                    ))
                                                }
                    ...
                    >
                        <Modal.Header closeButton>
                            <Modal.Title id="contained-modal-title-vcenter">Thông báo giỏ hàng</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                            Quý khách có chắc muốn loại bỏ sản phẩm này ra khỏi giỏ hàng
                        </Modal.Body>
                        <Modal.Footer>
                            <Button variant="secondary" onClick={handleCloseAlert} > không</Button>
                            <Button variant="primary" onClick={() => RemoveProduct()} >có</Button>
                        </Modal.Footer>
                    </Modal>
        </>
    );
