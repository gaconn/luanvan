    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (
                ...
                response.data[0].SoNha !== null
            ) {
                const addrress =
                    response.data[0].TinhThanh +
                    "," +
                    response.data[0].QuanHuyen +
                    "," +
                    response.data[0].PhuongXa +
                    "," +
            ...
            <>
                {user.SoDienThoai && (
                    <p className="mt-3 text-gray-700 text-sm ">
                        {" "}
                        Số điện thoại : <label>{user.SoDienThoai}</label>
                    </p>
                )}
                {user.NgaySinh && (
                    <p className="mt-3 text-gray-700 text-sm ">
                        {" "}
                        Ngày Sinh: <label>{formatDateForInput(user.NgaySinh * 1000)}</label>
                    </p>
                )}
                {DiaChi.addrress && (
                    <p className="mt-3 text-gray-700 text-sm ">
                        {" "}
                        Địa Chỉ : <label>{DiaChi.addrress}</label>
                    </p>
                )}
            </>
        )
                ...
                <div className="flex items-start px-4 py-6">
                    ...
                    <div>
                        ...
                        <p className="text-gray-700">
                            {formatDateForInput(user.ThoiGianTao * 1000)
                                ? formatDateForInput(user.ThoiGianTao * 1000)
                                : " "}{" "}
                        </p>
                        <p className="mt-3 text-gray-700 text-sm ">
