...
class CategoryModel {
    ...
    delete = async (objCondition) => {
        ...
        try {
            ...
            if (
                arrDataCheckResponse &&
                arrDataCheckResponse[0] &&
                arrDataCheckResponse[0].length > 0
            ) {
                return ResponseUtil.response(
                    false,
                    "Ngành hàng có chứa sản phẩm trong kho, không thể xóa."
                )
            }
            ...
            if (
                ...
                dataCheckSubCategory.data.data.length > 0
            ) {
                return ResponseUtil.response(
                    false,
                    "Ngành hàng có chứa ngành hàng con, không thể xóa. "
                )
            }
