...
class OrderModel {
    ...
    get = async (objCondition) => {
        ...
        try {
            ...
            if (objCondition.joinPaymentMethod) {
                ...
                var arrFieldPaymentMethodSelect = ["TenPhuongThucThanhToan"]

                strSelect += _buildSelect(
                    arrFieldPaymentMethodSelect,
                    "phuongthucthanhtoan",
                    "PhuongThucThanhToan_"
                )
            }
            ...
            const countResponse = await DBConnection.query(
                `select COUNT(id) rowCount from donhang ${strWhere}`
            )
    ...
    checkoutV2 = async (objData) => {
        ...
        try {
            ...
            const resultCheckout = this._checkout(
                { arrProduct: [productDetail], extraInfo },
                mailReceiver
            )
        ...
        try {
            ...
            if (productCount === 0) {
                return ResponseUtil.response(
                    false,
                    "Không có sản phẩm nào có thể đặt hàng",
                    [],
                    arrErrors
                )
            }
            ...
            if (mailReceiver) {
                ...
                if (html) {
                    const resultMailing = await sendMail(
                        mailReceiver,
                        ` Thông tin đơn hàng ${objOrder.MaDonHang}`,
                        html
                    )
                }
            }
            ...
            const messageTelegram = `Đơn hàng mới. \n Thời gian: ${new Date()}. \n Link quản lý: ${
                process.env.APP_URL
            }/manage/order/detail?id=${objOrderInfo.id}`
    ...
    checkoutV3 = async (objData) => {
        ...
        try {
            ...
            const dataCartItemResponse = await CartItemModel.getListCartItem({
                IDGioHang: objData.IDGioHang,
                IDSanPham: objData.IDSanPham,
            })

            if (
                !dataCartResponse.success ||
                dataCartResponse.data.length === 0 ||
                !dataCartItemResponse.success ||
                dataCartItemResponse.data.length === 0
            ) {
                return ResponseUtil.response(
                    false,
                    "Lấy thông tin giỏ hàng thất bại, hoặc không có sản phẩm nào trong giỏ hàng"
                )
            }
            ...
            const resultCheckout = await this._checkout(
                { arrProduct: arrProduct, extraInfo },
                mailReceiver
            )
            ...
            if (resultCheckout.success) {
                ...
                if (resultCheckout.error.length > 0) {
                    ...
                    for (let index = 0; index < arrListIDSanPham.length; index++) {
                        for (
                            let indexFail = 0;
                            indexFail < arrListIDSanPhamFail.length;
                            indexFail++
                        ) {
                ...
                if (listID) {
                    const removeCartItemResponse = await CartModel.removeFromCart({
                        IDSanPham: listID,
                        IDGioHang: objData.IDGioHang,
                    })
                }
    ...
    update = async (objData, objCondition) => {
        ...
        try {
            var objOrder = {
                ...
                IDPhuongThucThanhToan: objData.IDPhuongThucThanhToan
                    ? objData.IDPhuongThucThanhToan
                    : undefined,
                ThoiGianCapNhat: new Date().getTime() / 1000,
                DaXoa: objData.DaXoa ? objData.DaXoa : undefined,
                TongGiaTriDonHang: objData.TongGiaTriDonHang
                    ? objData.TongGiaTriDonHang
                    : undefined,
