...
class CartModel {
    ...
    addToCart = async (objData) => {
        ...
        try {
            ...
            if (dataCartItem.data.length > 0) {
                ...
                const resultUpdateCartItem = await CartItemModel.update({
                    IDSanPham: objData.IDSanPham,
                    SoLuong: sl,
                    id: dataCartItem.data[0].id,
                })
    ...
    removeFromCart = async (objCondition) => {
        ...
        try {
            ...
            if (
                !cartDataResponse ||
                !cartDataResponse.success ||
                cartDataResponse.data.length === 0
            ) {
            ...
            const dataCartItem = await CartItemModel.getListCartItem({
                IDGioHang: objCondition.IDGioHang,
            })
            var countItem = 0
            var quantity = 0
