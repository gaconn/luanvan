...
const User = {
    CheckToken: (req, res, next) => {
        ...
        if (!token) {
            return res.json(
                ResponseUtil.response(false, "Thao tác không hợp lệ", [], ["Token không tồn tại"])
            )
        }
        try {
            ...
            if (!decoded) {
                return res.json(
                    ResponseUtil.response(
                        false,
                        "Hãy chắc chắn rằng bạn đã đăng nhập để sử dụng dịch vụ này"
                    )
                )
            }
            ...
            return res.json(
                ResponseUtil.response(false, "Vui lòng đăng nhập lại để thực hiện thao tác")
            )
        }
    },
    googleLogin: (req, res) => {
        ...
        client.verifyIdToken(verifyObject).then((response) => {
